  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
        ...
        <input required onChange={onChangeHandler} name='address' value={formData.address} className='border border-gray-300 rounded py-1.5 px-3.5 w-full' type="text" placeholder='Adress'/>
        <div className='flex gap-3'>
        ...
        <input required onChange={onChangeHandler} name='phone' value={formData.phone} className='border border-gray-300 rounded py-1.5 px-3.5 w-full' type="number" placeholder='PhoneNumber'/>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            ...
            <div onClick={()=>setMethod('gpay')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'gpay' ? 'bg-green-400' : ''} `}></p>
                <img className='h-5 mx-4' src={assets.gpay_logo} alt="" />
            </div>
